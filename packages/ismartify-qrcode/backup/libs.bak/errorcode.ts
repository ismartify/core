 ...
 * 
 * 用于QR码的错误检测和纠正功能
 * 基于原始qr-image库的errorcode.js实现
 */

// 伽罗华域GF(256)的基础多项式：x^8 + x^4 + x^3 + x^2 + 1 = 285
const GF256_BASE = 285;

// 指数表：存储α^i的值，其中α是GF(256)的原根
const EXP_TABLE: number[] = [1];

// 对数表：存储log_α(i)的值，用于快速查找
...
(() => {
  ...
  for (let i = 1; i < 256; i++) {
    let n = EXP_TABLE[i - 1] << 1; // 乘以α (左移一位)
    if (n > 255) {
 ...
 * 
 * @param msg 原始消息数据（可以是Buffer或数组）
 * @param ecLen 纠错码长度
 * @returns 纠错码Buffer
 */
export function calculateErrorCode(msg: Buffer | number[], ecLen: number): Buffer {
  // 将消息转换为数组格式以便处理
